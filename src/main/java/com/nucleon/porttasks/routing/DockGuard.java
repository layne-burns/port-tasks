 ...
 * "dock", or a click on a port's gangplank) while holding a courier crate in hand (the caller checks), the port is identified by
 * location (the port whose water-side navigation point is nearest the boat) and the click is blocked if that
 * port has no pickup or delivery anywhere in the plan. As with {@link DepositGuard}, the menu is never
...
public final class DockGuard
{
	 ...
	 */
	public PortLocation wrongPort(RoutePlanner.Plan plan)
	{
		if (plan == null || plan.stops.isEmpty() || client.isKeyPressed(KeyCode.KC_SHIFT))
		{
			return null;
		}
		PortLocation port = nearestPort();
		if (port == null)
		{
			return null;
		}
